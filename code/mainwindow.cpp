...
void MainWindow::slotDeviceSaved(deviceInformationWidget* deviceinfo,bool b,int index)
{
    if(b)
    {
        ...
        devInfo->setPrimaryClass(deviceinfo->primaryModel);
        devInfo->setMinorClassCode(deviceinfo->monirModelCode);
        devInfo->setMinorClass(deviceinfo->monirModel);
...
void MainWindow::readDeviceIni()
{
    ...
    for (int i = 0; i < size; ++i)
    {
        ...
        devInfoList.at(i)->setPrimaryClass(deviceIni.value("primaryClass").toString());
        devInfoList.at(i)->setMinorClass(deviceIni.value("minorClass").toString());
        ...
        for(int j=0; j<sizefunction; ++j)
        {
            ...
            myFunlist.at(j)->setNewVarType(deviceIni.value("newVarType").toInt(), true);
...
void MainWindow::wrDeviceIni()
{
    ...
    for (int i = 0; i < devInfoList.size(); ++i)
    {
        ...
        deviceIni.setValue("primaryClass", devInfoList.at(i)->getPrimaryClass());
        deviceIni.setValue("minorClassCode", QString::number(devInfoList.at(i)->getMinorClassCode()));
        deviceIni.setValue("minorClass", devInfoList.at(i)->getMinorClass());
